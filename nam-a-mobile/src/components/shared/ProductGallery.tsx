...
const ProductGallery = ({ gallery }: { gallery: { src: string; alt: string }[] }) => {
  ...
  const navigateImage = (direction: number) => {
    const newIndex = (selectedImage + direction + gallery.length) % gallery.length;
    setSelectedImage(newIndex);
  };

  return (
    <div className="bg-white rounded-[10px] shadow-sm p-4 md:p-6">
      {/* Main image */}
      <div className="relative mb-4 group">
        <div className="relative w-full rounded-lg overflow-hidden bg-gray-100 aspect-video md:aspect-[4/3]">
          ...
          <button
            ...
            className="absolute bottom-3 right-3 bg-white/80 backdrop-blur-sm p-2 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity"
