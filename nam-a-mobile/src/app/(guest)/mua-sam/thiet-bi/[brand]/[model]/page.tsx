...
export async function generateStaticParams() {
  const models = await modelService.getModels({page: 1, pageSize: 100});
  return models.result?.data?.map((b) => ({
    brand: b.brandName,
    model: b.name,
  })) ?? [];
}
...
export default async function BrandListingPage({
  ...
  const products = await productService.getProductsByBrandandModelSlugs(brand, model);
  const modelName = products.result?.data && products.result?.data[0].modelName
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    { label: brand, href: `/mua-sam/thiet-bi/${brand.toLowerCase()}` }, 
    { label: modelName ?? model,  href: '/'  }
  ];
          ...
          <div className="bg-white p-4 my-4 rounded-2xl shadow-sm mb-6">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-xl font-bold text-gray-800">{modelName?.toUpperCase()} CŨ</h2>
                <p className="text-gray-600 mt-1">
                  Series {modelName} cũ chính hãng, chất lượng cao, giá tốt tại Nam Á Mobile
                </p>
              </div>
            </div>
          </div>
...
export async function generateMetadata({
  ...
  const brandCapital = brand.charAt(0).toUpperCase() + brand.slice(1);
  const modelCapital = model.toUpperCase();

  return {
    title: `Mua ${brandCapital} ${modelCapital} Cũ Giá Tốt | Nam Á Mobile Tân Phú`,
    description: `Mua ${brandCapital} ${modelCapital} cũ chính hãng tại Nam Á Mobile Tân Phú – Giá rẻ, bảo hành rõ ràng, hỗ trợ trả góp và giao hàng tận nơi. Sản phẩm đã qua kiểm định chất lượng.`,
    keywords: [
      `${brandCapital} ${modelCapital} cũ`,
      `${brandCapital} ${modelCapital} chính hãng`,
      `mua ${brandCapital} ${modelCapital} giá rẻ`,
      `Nam Á Mobile`,
      `điện thoại ${brand} cũ`,
      `${brandCapital} ${modelCapital} second-hand`,
      `điện thoại ${brand} đã qua sử dụng`,
      `điện thoại ${brand} ${model}`,
    ],
    openGraph: {
      title: `Mua ${brandCapital} ${modelCapital} Cũ Chính Hãng | Nam Á Mobile`,
      description: `Khám phá ${brandCapital} ${modelCapital} cũ chất lượng cao, giá tốt tại Nam Á Mobile. Bảo hành đầy đủ, giao hàng nhanh toàn quốc.`,
      url: `https://namamobile.vn/thiet-bi/${brand}/${model}`,
      type: "website",
    },
  };
}
